...
describe('PlantsService', () => {
  ...
  describe('findOne', () => {
    it('should return plant details when found', async () => {
      ...
      expect(plantsRepository.findByIdWithLastLog).toHaveBeenCalledWith(
        userId,
        plantId,
      )
    ...
    it('should throw error when plant is not found', async () => {
      ...
      await expect(service.findOne(userId, plantId)).rejects.toThrow(
        'Plant not found',
      )
    })
  })
  ...
  describe('softDelete', () => {
    it('should soft delete plant after existence check', async () => {
      ...
      expect(plantsRepository.findOne).toHaveBeenCalledWith(userId, plantId)
      expect(plantsRepository.softDelete).toHaveBeenCalledWith(userId, plantId)
    })
  })
